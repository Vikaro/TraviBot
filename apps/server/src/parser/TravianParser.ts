...
export function parseResourcesPage(res: Response): { actualQueue: Array<Building>, resourceBuildings: Array<Building> } {
    const $res = $(res.text);
...
export function parseAdventuresPage(res: Response) {
    const $res = $(res.text);
    const heroVillageId = $res.find(".heroStatusMessage.header a")
        .attr("onclick")
    ...
    adventuresTable.each((index, el) => {
        ...
        const difficult = $el.find(".difficulty > img").hasClass("adventureDifficulty0") ? "Hard" : "Normal";
        const id = $el.find(".goTo > a").attr("href").replace('a2b.php?id=', '').replace('&h=1', '');
...
export function parseAdventurePage(res: Response) {
    ...
    const params = inputs.map((index, el) => {
        const $el = $(el);
