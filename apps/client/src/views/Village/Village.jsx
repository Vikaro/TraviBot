...
class Village extends Component {
  ...
  render() {
    ...
    const resourceBuildings = Object.values(buildings).filter(i => parseInt(i.id) < 19);
    const villageBuildings = Object.values(buildings).filter(i => parseInt(i.id) >= 19);
    const nonMaxLevelBuildings = Object.values(buildings).filter(i => i.level !== 'max');
          ...
          <GridItem xs={12} sm={12} md={6}>
            <Card>
              <CardHeader color="warning">
                ...
                <p className={classes.cardCategoryWhite}>Upgradeable buildings: {nonMaxLevelBuildings.length}</p>
              </CardHeader>
              <CardBody>
                <BuildingsPanel title="Resource Buildings" buildings={resourceBuildings} classes={classes} />
                <BuildingsPanel title="Village Buildings" buildings={villageBuildings} classes={classes} />
                <RegularButton onClick={this.handleAutoBuildVillage}> Auto Build village</RegularButton>
              </CardBody>
            </Card>
          </GridItem>
          <GridItem md={6}>
            <Card>
              <CardHeader color="warning">
                ...
                <p className={classes.cardCategoryWhite}>Found: {smithyUpgrades.length}</p>
              </CardHeader>
              <CardBody>
                {!isAnyUpgrade && <Typography>Can't find any upgrade left. Do you have smithy?</Typography>}
                {isAnyUpgrade &&
                  <SmithyPanel title="Upgrades" smithyUpgrades={smithyUpgrades} classes={classes} />}
              </CardBody>
              <CardFooter>
                {isAnyUpgrade &&
                <RegularButton onClick={this.handleAutoUpgradeUnits}>Auto upgrade units</RegularButton>}
              </CardFooter>
            </Card>
          </GridItem>
          <GridItem md={6}>
            <Card>
              ...
              <CardBody>
                {units.map(unit => 
                  <Typography key={unit.id}> Name : {unit.name}  // {unit.available} </Typography>
                )}
              </CardBody>
            </Card>
          </GridItem>
...
const BuildingsPanel = ({ title, buildings, classes }) => {
  ...
  return (
    <ExpansionPanel>
      <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
        <Typography className={classes.heading}>{title} ({nonMaxLevelBuildings.length})</Typography>
      </ExpansionPanelSummary>
      <ExpansionPanelDetails>
        <List component="nav">
          {buildings.map(building =>
            <ListItem button key={building.id}>
              ...
              <ListItemText primary={building.name} secondary={building.level} />
            </ListItem>
          )}
        </List>
      </ExpansionPanelDetails>
        ...
        <List component="nav">
          {smithyUpgrades.map(upgrade =>
            <ListItem button key={upgrade.id}>
              ...
              <ListItemText primary={upgrade.title} secondary={upgrade.duration} />
            </ListItem>
          )}
        </List>
      </ExpansionPanelDetails>
